...
pip install -r services/telegram-service/requirements.txt
...
if [ ! -f "$ENV_DIR/.env" ]; then
  echo "TELEGRAM_BOT_TOKEN=你的token" > "$ENV_DIR/.env"
  echo "⚠️  请编辑 $ENV_DIR/.env 填入真实 Token"
fi
...
Type=simple
User=$USER
WorkingDirectory=$DEPLOY_DIR/services/telegram-service
